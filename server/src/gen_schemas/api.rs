...
pub struct GameState<'raw> {
    ...
    pub drawing_a: ::std::vec::Vec<Stroke<'raw>>,
    pub drawing_b: ::std::vec::Vec<Stroke<'raw>>,
}
...
impl<'raw> ::bebop::SubRecord<'raw> for GameState<'raw> {
    ...
    fn serialized_size(&self) -> usize {
        self.id.serialized_size()
            ...
            + self.drawing_a.serialized_size()
            + self.drawing_b.serialized_size()
    }
    ...
    ::bebop::define_serialize_chained!(Self => |zelf, dest| {
        Ok(
            ...
            zelf.drawing_a._serialize_chained(dest)? +
            zelf.drawing_b._serialize_chained(dest)?
        )
    });
...
#[derive(Clone, Debug, PartialEq, Copy)]
#[repr(packed)]
pub struct ClientTypeDTO {
    pub gamer_id: u8,
    pub id: u32,
    pub ctype: ClientType,
}

impl ::bebop::FixedSized for ClientTypeDTO {}

impl<'raw> ::bebop::SubRecord<'raw> for ClientTypeDTO {
    const MIN_SERIALIZED_SIZE: usize = Self::SERIALIZED_SIZE;
    const EXACT_SERIALIZED_SIZE: Option<usize> = Some(Self::SERIALIZED_SIZE);
    ...
    ::bebop::define_serialize_chained!(*Self => |zelf, dest| {
        Ok(
            ::bebop::packed_read!(zelf.gamer_id)._serialize_chained(dest)? +
            ::bebop::packed_read!(zelf.id)._serialize_chained(dest)? +
            ::bebop::packed_read!(zelf.ctype)._serialize_chained(dest)?
        )
    });
...
impl<'raw> ::bebop::Record<'raw> for ClientTypeDTO {}
...
pub mod owned {
    ...
    pub struct GameState {
        ...
        pub drawing_a: ::std::vec::Vec<Stroke>,
        pub drawing_b: ::std::vec::Vec<Stroke>,
    }
